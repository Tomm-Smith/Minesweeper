...
class Minefield_Obj:
    """ Object for syncronizing minefield, mines, and relative GUI states therein. 

    Class Arguments:
        field_size (int) - Size of the minefield matrix

    field = [[
                {'mine': 0, 'mine_coords': [0, 0], 'mask_id': None, 'block_wdgt': None, 'block_state': None},
                {'mine': 0, 'mine_coords': [0, 0], 'mask_id': None, 'block_wdgt': None, 'block_state': None},
                {'mine': 0, 'mine_coords': [0, 0], 'mask_id': None, 'block_wdgt': None, 'block_state': None}
            ],
            [
    ...
    Structure Reference:
        ...
        field_img_id - Image id of item on the minefield, None otherwise
    ...
    def delete_mask(self, x, y):
        ...
        if self.field[y][x]['mask_id'] != None:
            self.minefield_wdgt.delete(self.field[y][x]['mask_id'])
        
    def delete_block(self, x, y):
        # Attempt block widget destruction and handle exceptions
        try:
            field[y][x]['block_wdgt'].destroy()
        except IndexError:
...
class MineSweeper:
    ...
    def __gui__(self):
        ...
        if debug:
            ...
            self.field_obj_menu.add_command(label="place_mine()", command=lambda : self.mf.place_mine(3, 2))
    ...
    def minefield_press(self, event=None):
        pass
    ...
    def draw_field(self):
        ...
        for y in range(len(self.mf.field)):
            for x in range(len(self.mf.field)):
                ...
                # Create new one
                iid = self.minefield.create_image(field_x, field_y, image=img_src)
